...
const PromptBox = ({ isloading, setIsloading }) => {
    ...
    const sendPrompt = async (e) => {
        ...
        try {
            ...
            if (isloading) return toast.error('Wait for the previos prompt response')
            setIsloading(true)
            setPrompt("")
            ...
            if(data.success){
                setChats((prevChats)=>prevChats.map((chat)=>chat._id === selectedChat._id?{...chat,message:[...chat.message,data.data]}:chat))
    ...
    return (
        ...
        className={`w-full ${selectedChat?.message.length ? "max-w-3xl" : "max-w-2xl"} bg-[#404045] p-4 rounded-3xl mt-4 transition-all`}>
